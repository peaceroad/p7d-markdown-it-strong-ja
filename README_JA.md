...
- ベースライン：プレーンな `markdown-it` 区切り文字ペアから開始します。
- ローカルヘルパーパス：ローカルの日本語コンテキストで `*` のみが実行され、strong-ja 境界ロジックに入ります。
- 混合テキストガード：`japanese-boundary-guard` は、日本語と英語が混在する過剰変換をさらに抑制します。
- 後処理：トークンのみの修復は、不正なリンク/参照隣接範囲に対してのみ実行されます。
...
- `mode` と `postprocess` は runtime-effective です。`mditAttrs`、`patchCorePush`、`coreRulesBeforePostprocess` は最初の `.use(...)` 後は setup-time effective のままです。
