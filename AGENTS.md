...
- `src/token-postprocess/orchestrator.js`: postprocess runtime flow, stage dispatch, and registration.
- `src/token-postprocess/broken-ref.js`: broken-ref repair pass runner and wrapper-balanced segment handling.
...
- Per-inline postprocess now builds one shared facts object and runs explicit stages: activation, broken-ref repair, emphasis repair/sanitize, and collapsed-ref finalize.
...
- Broken-ref runtime flow now lives in `src/token-postprocess/broken-ref.js`; the orchestrator delegates the repair stage to that module.
- Broken-ref candidate rewrite now flows through named segment-end, guard, and fast-path decision helpers inside `broken-ref.js`, which keeps the pass runner thinner.
- Broken-ref and collapsed-ref stages both use named orchestrator gate/apply helpers, while candidate scanning and token surgery stay in `broken-ref.js` / `token-link-utils.js`.
...
- Per-inline facts lazily cache `code_inline`, reference count, metrics, `link_open -> link_close` maps, wrapper-prefix stats, and earliest-changed level-rebuild index.
...
- Map diagnostics: `npm run test:map` (reports structural diffs separately from map-quality regressions/improvements; pass `-- --show-structural` to inspect benign structural-only rewrites)
