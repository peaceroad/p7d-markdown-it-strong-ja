...
- `markdown-it` 15 keeps the code-point-aware astral delimiter behavior introduced in 14.2, and that behavior is preserved in `compatible` mode. Japanese modes add code-point-aware CJK context detection for astral Han (for example `*𠀋?*abc*` -> `<em>𠀋?</em>abc*`) but do not treat emoji/symbol-only English contexts as Japanese context (for example `*😀?*abc*` stays markdown-it-like).
